...
const DesignElement: FunctionComponent<DesignElementProps> = (props) => {
  ...
  const [height, setHeight] = React.useState<number>(textContainerRef.current?.clientHeight ?? 250)
  const [width, setWidth] = React.useState<number>(textContainerRef.current?.clientHeight ?? 250)
  const [text, setText] = React.useState<string>(props.designElement?.text ?? 'New Text')
  const [fontFace, setFontFace] = React.useState<FontFace>(props.designElement?.fontFace ?? fonts[0])
  ...
  const [flickerOn, setFlickerOn] = React.useState<boolean>(props.designElement?.flickerOn ?? false)
  ...
  const setDesignElement = (designElement: DesignElementType) => {
    setHeight(designElement.size?.height?? height)
    setWidth(designElement.size?.width ?? width)
    setText(designElement.text ?? text)
    setFontFace(designElement.fontFace ?? fontFace)
    setFontSize(designElement.fontSize ?? fontSize)
    setFlickerStyle(designElement.flickerStyle ??flickerStyle)
    setFlickerOn(designElement.flickerOn ?? flickerOn)
    setColor(designElement.color ?? color)
    setLayer(designElement.layer ?? layer)
  }

  React.useEffect(() => {
    if (textContainerRef.current) {
      console.log("size of the text",textContainerRef.current?.clientHeight,
        textContainerRef.current?.clientWidth)
      setDesignElement({
        ...props.designElement,
        size: {
          height: textContainerRef.current?.clientHeight ?? height,
          width: textContainerRef.current?.clientWidth ?? width
        }
      })
      setHeight(textContainerRef.current?.clientHeight)
      setWidth(textContainerRef.current?.clientWidth)
    }

  }, [textContainerRef, height, props.designElement, width, setDesignElement])
  ...
  React.useEffect(() => {
    if (props.designElement) {
      setDesignElement(props.designElement)
    }
  }, [props.designElement, setDesignElement])
    ...
    <Grid container item
      ...
      <Grid container item>
        <Grid
          container
          item>
          <Typography ref={textContainerRef} className={flickerOn ? flickers[flickerStyle].neonText : ''}
                      style={{...fontFace, fontSize: `${fontSize * FONT_MULTIPLIER}px`,
                        height: height + 'px',
                        width: width + 'px'}}>{text}</Typography>
        </Grid>
      </Grid>
      <Grid container item style={{position: 'absolute', bottom: 0, right: 0, padding: NeonTheme.spacing(0, 2)}}
            justifyContent="space-between">
        <Grid item>
          <Typography variant="subtitle1">X: {Math.round(props.designElement?.x ?? 0)}</Typography>
        </Grid>
        <Grid item>
          <Typography variant="subtitle1">Y: {Math.round(props.designElement?.y ?? 0)}</Typography>
        </Grid>
      </Grid>
    </Grid>
  )
...
//https://www.freeprivacypolicy.com/live/2431d79f-e464-4e7a-8e63-351afcdfc9cb
...
//   </li>
//   <li>
//     <p><strong>Company</strong> (referred to as either &quot;the Company&quot;, &quot;We&quot;, &quot;Us&quot; or &quot;Our&quot; in this Agreement) refers to Anybody Walking.</p>
//   </li>
//   <li>
...
//   </li>
//   <li>
//     <p><strong>Website</strong> refers to Anybody Walking, accessible from <a href="www.anybodywalking.com" rel="external nofollow noopener" target="_blank">www.anybodywalking.com</a></p>
//   </li>
//   <li>
...
// <ul>
//   <li>By email: ofwonderconsulting@gmail.com</li>
// </ul>